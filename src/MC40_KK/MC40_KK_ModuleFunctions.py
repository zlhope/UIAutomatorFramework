# ----------------------------------------------------------------------------------------------------------------------
# Name Of File: MC40_ModuleFunctions.py                                                                                #
# Author: Ashish Kumar                                                                                                 #
...
import unittest
import time
import MC40_KK_Config
import ModuleFunctions
Config=MC40_KK_Config.Config()
...
# ----------------------------------------------------------------------------------------------------------------------
    def scanhardKey(self):
        try:
            ...
            #below line for TC55
            cmd=self.command+" sendevent dev/input/event3 4 4 2;sendevent dev/input/event3 1 528 0;sendevent dev/input/event3 0 0 0;"
            
            os.system(cmd)
            time.sleep(2)
            #key up kernel event for scan button
            #below line for TC55
            cmd=self.command+" sendevent dev/input/event3 4 4 2;sendevent dev/input/event3 1 528 1;sendevent dev/input/event3 0 0 0;"
